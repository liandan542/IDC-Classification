...
earlystopping = EarlyStopping(monitor='val_loss', min_delta=0, patience=40, verbose=0, mode='auto')
checkpoint = ModelCheckpoint(weight_path, monitor='val_loss', verbose=1, save_best_only=True, mode='min', save_weights_only=True)
tensorboard = TensorBoard(log_dir="./logs/{}".format(time()))

model.compile(loss = 'binary_crossentropy', optimizer = sgd, metrics = ['accuracy'])

hist = model.fit(x_train, y_train, batch_size=512, epochs=80, callbacks = [earlystopping, checkpoint, tensorboard], validation_split = 0.3)
...
plt.title('Test accuracy : ' + str(acc))
plt.ylabel('Accuracy')
plt.xlabel('Epoch')
